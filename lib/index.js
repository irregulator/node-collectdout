...
function Collectd(interval, host, port, hostname) {
        ...
        case 'object':
            for (var i=0; i < host.length; i++) {
                ...
                this.serverHosts[this.serverHosts.length] = { 'host': h[0], 'port': (h[1] || 25826) 
                                                              'securitylevel': (h[2] || 0),
                                                              'username': (h[3] || undefined),
                                                              'password': (h[4] || undefined) };
                this.usrMaxLen = Math.max(Buffer.byteLength(h[3]), this.usrMaxLen);
                this.maxSecLvl = Math.max(h[2].length, this.maxSecLvl);
            }
            break;
        case 'string':
    ...
    /* Calculates overhead according to maximum security level and
    *  maximum username length for given set of server hosts. */
    this.overhead = [0, 36, 42][this.maxSecLvl] + this.usrMaxLen;
...
Collectd.prototype.send = function() {
    ...
    var pkt = new Packet(this.write.bind(this), this.overhead.bind(this));
...
Collectd.prototype.write = function(buf) {
    for (var i=0; i < this.serverHosts.length; i++) {
        (function(dat) {
            if (this.serverHosts[i].securitylevel == 1) {
                dat = this.sign(this.serverHosts[i].username, this.serverHosts[i].password, dat);
            }
            this.serverHosts[i].sock.send(dat, 0, dat.length, this.serverHosts[i].port, this.serverHosts[i].host);
        })(buf);
    }
};

Collectd.prototype.sign = function(usr, pas, dat) {

    var plen = 2 + 2 + 32 + usr.length;

    if (!Buffer.isBuffer(usr))
        usr = new Buffer(usr);

    dat.writeUInt16BE(0x0200, 0);
    dat.writeUInt16BE(plen, 2);

    hmac = crypto.createHmac('sha256', pas);
    hmac.write(usr + dat);
    hmac.end();
    hash = hmac.read();
    hash.copy(dat, 4);
...
function Packet(sendCb, overhead) {
    this.sendCb = sendCb;
...
Packet.prototype = {
    send: function() {
	if (this.pos > this.overhead)
	    this.sendCb(this.buf.slice(0, this.pos));
	this.pos = this.overhead;
    },
